...
def parse_vk():
    ...
    gather_followers(api,adjacency_list,mem_list)
    gather_friends(api,adjacency_list,mem_list) 

    write_to_file_adj(adjacency_list, str(group_id)+'adj.txt')
    #работа с графом (по списку смежности)
    group_graf(adjacency_list)

def gather_followers(api,adjacency_list,mem_list):
...
def gather_friends(api,adjacency_list,mem_list):
    in_index = 0 #из вксрипта
    ex_index = 0 #здесь
